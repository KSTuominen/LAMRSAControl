##' MRSA_model_4_parameter
##'
##' @importFrom SimInf mparse
##' @importFrom utils data
##' @param Nt The daily decay rate (The proporation of phi that is
##'     left after one day's decay)
##' @param beta_phi Not used
##' @param beta_phi_mature The indirect transmission rate for sows and gilts
##' @param beta_phi_piglets The indirect transmission rate for piglets
##' @param beta_phi_growers The indirect transmission rate for growers
##' @param beta_phi_finish The indirect transmission rate for finishers
...
##' @export
MRSA_model_4_parameter <- function(Nt=0.8705506,
                                   beta_phi = 0.00285,
                                   beta_phi_mature = 0.0008,
                                   beta_phi_piglets = 0.0008,
                                   beta_phi_growers = 0.0008,
                                   beta_phi_finish = 0.0008,
                                   duration = 17.4,
    ...
    gdata <- c(Nt = Nt,
               ...
               Se = Se,
               alpha = alpha,
               beta_phi_mature = beta_phi_mature,
               beta_phi_piglets = beta_phi_piglets,
               beta_phi_growers = beta_phi_growers,
               beta_phi_finish = beta_phi_finish)
    ...
    transitions <-  c( ## Transitions for testing and the effect of that:
        ...
        "Ssows -> (1 - result_indicator) * Ssows * beta_phi_mature * (phi + s_phi * room_scale + f_phi * room_scale * farm_scale) -> Isows1 + Icum",
        "Sgilts -> (1 - result_indicator) * Sgilts * beta_phi_mature * (phi + s_phi * room_scale + f_phi * room_scale * farm_scale) -> Igilts1 + Icum",
        "Spiglets -> (1 - result_indicator) * Spiglets * beta_phi_piglets * (phi + s_phi * room_scale + f_phi * room_scale * farm_scale) -> Ipiglets1 + Icum",
        "Sgrowers -> (1 - result_indicator) * Sgrowers * beta_phi_growers * (phi + s_phi * room_scale + f_phi * room_scale * farm_scale) -> Igrowers1 + Icum",
        "Sfinish -> (1 - result_indicator) * Sfinish * beta_phi_finish * (phi + s_phi * room_scale + f_phi * room_scale * farm_scale) -> Ifinish1 + Icum",
        sows$transitions,
        gilts$transitions,
        piglets$transitions,
        growers$transitions,
